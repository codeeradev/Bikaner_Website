...
const content: Record<string, { title: string; field: keyof PublicSettings }> = { about: { title: 'About Us', field: 'aboutUs' }, privacy: { title: 'Privacy Policy', field: 'privacyPolicy' }, terms: { title: 'Terms & Conditions', field: 'termsAndConditions' }, shipping: { title: 'Shipping Policy', field: 'shippingPolicy' }, refund: { title: 'Refund Policy', field: 'refundPolicy' } };
export default function InfoPage({ params }: { params: { page: string } }) {
  ...
  return <main className="page-content info-page"><p className="eyebrow">Bikaner Bakery</p><h1>{info.title}</h1><div className="info-content">{String(body).split(/\n+/).map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div></main>;
}
